...
class Pipeline:
            ...
            if parsed.actions:
                ...
                for action in parsed.actions:
                    try:
                        ...
                        if self.robot and self.config.robot.enabled:
    ...
    def _maybe_inject_vision(self, user_text: str) -> str:
        """Inject screen or camera vision based on user request, or random screen glance.

        Webcam is ONLY used when the user explicitly asks (camera keywords).
        Random vision checks are screen-only.
        """
        ...
        # Check if user explicitly asked about screen
        screen_triggered = any(kw in text_lower for kw in self._SCREEN_KEYWORDS)
        if screen_triggered:
            return self._inject_screen(user_text)

        # Random 20% chance — screen only, never webcam
        # Random 20% chance — screen only, never webcam
        if random.random() < 0.2:
            if self.screen and self.screen.available:
                return self._inject_screen(user_text)
