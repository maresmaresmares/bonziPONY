...
class ConversationConfig:
    ...
    listen_timeout_s: float = 8.0    # seconds to wait for user to START speaking in follow-up
...
def load_config(path: Path | str = "config.yaml") -> AppConfig:
    ...
    llm_api_key = os.environ.get("BONZI_LLM_API_KEY") or llm_raw.get("api_key", "")
    llm_provider = os.environ.get("BONZI_LLM_PROVIDER") or llm_raw.get("provider", "openai")
    llm_base_url = os.environ.get("BONZI_LLM_BASE_URL") or llm_raw.get("base_url")
    el_api_key = os.environ.get("BONZI_ELEVENLABS_API_KEY") or el_raw.get("api_key", "")
    el_voice_id = os.environ.get("BONZI_ELEVENLABS_VOICE_ID") or el_raw.get("voice_id", "")
